...
/* ── Early PWA prompt on load (desktop + mobile) ── */
(function _p9_installPromptOnOpen() {
  const KEY = 'p9_install_banner_seen';
  const isStandalone = window.matchMedia?.('(display-mode: standalone)').matches || window.navigator.standalone;
  if (isStandalone || sessionStorage.getItem(KEY) === '1') return;

  let bannerShown = false;

  function closeBanner() {
    sessionStorage.setItem(KEY, '1');
    document.getElementById('p9InstallBanner')?.remove();
  }

  function showBanner() {
    if (bannerShown || document.getElementById('p9InstallBanner') || isStandalone) return;
    bannerShown = true;
    const bn = document.createElement('div');
    bn.id = 'p9InstallBanner';
    ...
    bn.innerHTML = `
      ...
      <div class="pwa-banner-text">
        ...
        <div class="pwa-banner-sub">تثبيت الآن أو لاحقًا — تجربة أسرع على الهاتف والكمبيوتر</div>
      </div>
    ...
    setTimeout(() => { if (document.getElementById('p9InstallBanner')) closeBanner(); }, 15000);
  }

  const ready = () => setTimeout(showBanner, 900);
...
console.log('✅ patch_v9.js v9.3 — Session Exit Visible + Responsive Admin + PWA Install UX');
