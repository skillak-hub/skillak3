    ...
    async function adTab(tab, el) {
      ...
      if (tab === 'users') {
        ...
        con.innerHTML = `
          ...
          const financeState = b.status === 'completed' ? (paid ? 'تم التحويل' : 'جاهز للتحويل') : (b.status === 'refunded' ? 'مسترد' : '—');
          const financeBadge = b.status === 'completed' ? (paid ? 'pc' : 'pp') : (b.status === 'refunded' ? 'pca' : 'pp');
          const sessionBrief = `
          ...
          return `<tr>
                ...
                <td>
                  <div style="display:flex;gap:4px;flex-wrap:wrap">
                    ...
                    ${b.status === 'completed' ? `<button class="btn btn-o btn-xs" onclick="adminRefundBk('${b.id}','${b.studentId}',${b.total || 0})">↩️ إرجاع للطالب</button>` : ''}
